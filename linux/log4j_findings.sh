...
handle_war_ear_zip()
{
	...
	if jar1=`unzip -l $war_file | awk '{print $NF}'| grep -i ".jar" 2> /dev/null `;then
		...
		unzip -Bd /tmp/log4j_for_extract/ $war_file > /dev/null
	fi;
...
log4j()
{
    ...
    if [ $NETDIR_SCAN = true ];then
	    ...
	    jars=$(find ${BASEDIR} -xautofs -fstype nfs -prune -o -fstype nfs4 -prune -o -fstype cifs -prune -o -fstype smbfs -prune -o -fstype gfs -prune -o -fstype gfs2 -prune -o -fstype safenetfs -prune -o -fstype secfs -prune -o -fstype gpfs -prune -o -fstype smb2 -prune -o -fstype vxfs -prune -o -fstype vxodmfs -prune -o -fstype afs -prune -o -type f -regextype posix-egrep -iregex ".+\.(jar|war|ear|zip)$" -print 2>/dev/null);
    fi
